...
def cnn_mobile(x):
    with tf.name_scope("model"):
        with tf.variable_scope("layer1") as scope:
            ...
            max_pool_1,_ = max_pool_2x2(conv2_1)  # out 14*14*32


        with tf.variable_scope("layer2") as scope:
            # layer2 - mobile_net
            weight2_1 = weights([5, 5, 32, 1])
            bas_2_1 = baise([32])
            conv2_21 = tf.nn.relu(conv2d_depth(max_pool_1, weight2_1) + bas_2_1,name=scope.name)  # 14*14*64
            weight2_2 = weights([1, 1, 32, 64])
            bas_2_2 = baise([64])
            conv2_22 = tf.nn.relu(conv2d(conv2_21, weight2_2) + bas_2_2)
            #conv2_22 = tf.nn.separable_conv2d(max_pool_1,weight2_1,weight2_2,[1,1,1,1])
            max_pool_2, _ = max_pool_2x2(conv2_22)  # 7*7*64
...
#v_layer1,v_layer2,y_pre1 = cnn(images)
y_pre = cnn_mobile(images)
