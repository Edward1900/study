...
if isfile('../input/w/standard13.model'):
    ...
    for _ in range(2):
        make_steps(5, 0.2)
        model.save('standard14.model')
    set_lr(model, 1e-6)
    for _ in range(2):
        make_steps(5, 0.2)
        model.save('standard15.model')
    # set_lr(model, 2e-7)
    # for _ in range(2):
    #     make_steps(5, 0.2)
    #     model.save('standard14.model')
...
tmp = keras.models.load_model('../input/w/mpiotte-bootstrap.model')
model.set_weights(tmp.get_weights())
...
# Evaluate the model.
fknown2 = branch_model.predict_generator(FeatureGen(known), max_queue_size=20, workers=10, verbose=0)
fsubmit2 = branch_model.predict_generator(FeatureGen(submit), max_queue_size=20, workers=10, verbose=0)
score2 = head_model.predict_generator(ScoreGen(fknown2, fsubmit2), max_queue_size=20, workers=10, verbose=0)
score2 = score_reshape(score2, fknown2, fsubmit2)

# a1 = score[1, :]
# t = np.argsort(a1)
# print(a1[list(reversed(t))])
# Generate the subsmission file.




score = 0.55*score1 + 0.45*score2
# Generate the subsmission file.
prepare_submission(0.87, 'submission_0.55_standard_0.45_boostrap(0.87).csv')
prepare_submission(0.85, 'submission_0.55_standard_0.45_boostrap(0.85).csv')

score = 0.6*score1 + 0.4*score2
# Generate the subsmission file.
prepare_submission(0.85, 'submission_0.6_standard_0.4_boostrap(0.85).csv')
