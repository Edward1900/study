...
#gc.collect()
# model.save("model.h5")
#model.load_weights('model_epoch-19_loss-0.0430_val_loss-2.4374.h5')
...
def map5(X, y):
    ...
    for i in range(X.shape[0]):
        ...
        for j in range(pred.shape[0]):
            # print(pred[j])
            # print(np.argwhere(y[i]==1))
            if pred[j] ==  (int(np.argwhere(y[i]==1))-1):
                score += (5 - j)/5
                break
...
sco1 = map5(predictions,y)

print("org:",sco1)

best_th = 0
best_score = 0
for th in np.arange(0.1, 0.700, 0.05):
    predictions[:,0] = th
